...
Commits regulares durante el desarrollo, no al final. Cada funcionalidad tiene su propio commit antes de continuar con la siguiente.

### Claridad de Mensajes de Commit
Convención `feat/fix/chore/refactor(scope): descripción`:
```
feat(domain): agregar entidades y contratos de repositorios
feat(application): implementar RegistrarPedidoUseCase con transaccion
feat(infra): configurar Polly Circuit Breaker para servicio externo
fix(infra): remover EnableRetryOnFailure incompatible con UnitOfWork
```

### Organización del Repositorio
`.gitignore` excluye `bin/`, `obj/`, `.vs/`, secrets. El `README.md` es la documentación principal. El script SQL está en `/database`. Los proyectos siguen la convención `NombreSolucion.Capa`.
...
git add src/PedidosAPI.Domain/
git commit -m "feat(domain): agregar entidades PedidoCabecera, PedidoDetalle y LogAuditoria"

git add src/PedidosAPI.Domain/Interfaces/
git commit -m "feat(domain): definir contratos IPedidoRepository e IAuditoriaRepository"

git add src/PedidosAPI.Application/DTOs/ src/PedidosAPI.Application/Exceptions/
git commit -m "feat(application): agregar DTOs de request/response y excepciones de dominio"

git add src/PedidosAPI.Application/Validators/
git commit -m "feat(application): implementar validaciones con FluentValidation"

git add src/PedidosAPI.Application/
git commit -m "feat(application): implementar RegistrarPedidoUseCase con logica transaccional"

git add src/PedidosAPI.Infrastructure/Data/
git commit -m "feat(infra): configurar AppDbContext con Fluent API y UnitOfWork transaccional"

git add src/PedidosAPI.Infrastructure/Repositories/
git commit -m "feat(infra): implementar PedidoRepository y AuditoriaRepository con EF Core"

git add src/PedidosAPI.Infrastructure/ExternalServices/
git commit -m "feat(infra): implementar ValidacionClienteService con HttpClient tipado"

git add src/PedidosAPI.Infrastructure/DependencyInjection/
git commit -m "feat(infra): configurar pipeline Polly Circuit Breaker + Retry + Timeout"

git add src/PedidosAPI.API/Endpoints/ src/PedidosAPI.API/Middleware/
git commit -m "feat(api): agregar Minimal API endpoints y ExceptionMiddleware"

git add src/PedidosAPI.API/Program.cs src/PedidosAPI.API/appsettings.json
git commit -m "feat(api): configurar Scalar, OpenAPI y pipeline de middlewares"

git add src/PedidosAPI.Application/UseCases/RegistrarPedidoUseCase.cs
git commit -m "feat(application): agregar ObtenerTodosAsync para listado de pedidos"

git add database/script.sql README.md
git commit -m "docs: agregar script SQL y documentacion completa del proyecto"
```
